...
use StendenINF1I\WaterUpApi\Exception\InvalidTypeCastException;

abstract class BaseContainer implements ContainerContract, \ArrayAccess, \Countable, \IteratorAggregate
{
     ...
     *
     * @return null|ContainerContract
     */
    public function safeSet(string $index, $value = null): ?ContainerContract
    {
        if (!$this->has($index))
        {
     ...
     */
    public function set(string $index, $value = null): ContainerContract
    {
    ...
    {
        if (is_a($exception, "\\WaterUpApi\\Exception\\InvalidTypeCastException") && $this->getThrowInvalidCastExceptions())
        {
            throw $exception;
        }

        if (is_a($exception, "\\InvalidArgumentException") && $this->getThrowSafeModeExceptions())
        {
            throw $exception;
        }
    }
     ...
     */
    public function setThrowInvalidCastExceptions(bool $throwInvalidCastExceptions): BaseContainer
    {
     ...
     * @return BaseContainer
     */
    public function setThrowSafeModeExceptions(bool $throwSafeModeExceptions): BaseContainer
    {
     ...
     * @return null|ContainerContract
     */
    public function safeAdd(array $parameters): ?ContainerContract
    {
     ...
     * @return ContainerContract
     */
    public function add(array $parameters): ContainerContract
    {
     ...
     * @return ContainerContract
     */
    public function replace(array $parameters): ContainerContract
    {
        $this->parameters = $parameters;
    }
    ...
    public function getDateTime(string $index, string $dateTimeFormat = null, \DateTime $default = null): ?\DateTime
    {
        if ($this->has($index))
        {
            ...
            catch (\Exception|\ErrorException $malformedDateTimeException)
            {
    ...
    public function getObject(string $index, object $default = null): ?object
    {
        if ($this->has($index))
        {
            ...
            catch (\Exception|\ErrorException $invalidCastException)
            {
                $this->throw(new InvalidTypeCastException(sprintf(
    ...
    public function getString(string $index, string $default = null): ?string
    {
        if ($this->has($index))
        {
            ...
            try
            {
                if ((!is_array($storedItem)) && ((!is_object($storedItem) && settype($storedItem, 'string') !== false) || (is_object($storedItem) && method_exists($storedItem, '__toString'))))
                {
                    return (string)$storedItem;
                }
                throw new \ErrorException(sprintf("Error: %s %s can not be converted to an string.", $index, gettype($storedItem)));
            }
            catch (\Exception|\ErrorException $invalidCastException)
            {
                $this->throw(new InvalidTypeCastException(sprintf(
    ...
    /**
     * This method will get an stored item from the container as an integer, its an alias for the getInt method.
     *
     * @param string   $index   The items index.
    ...
    {
        return $this->getInt($index, $default, $options);
    }
     ...
     * @return int|null
     */
    public function getInt(string $index, int $default = null, array $options = []): ?int
    {
        if ($this->has($index))
        {
            $storedItem = $this->get($index, $default);
     ...
     *
     * @param string                 $index   The items index.
     * @param ContainerContract|null $default The default return value if the item does not exist in the container.
     *
     * @return null|ContainerContract
     */
    public function getContainer(string $index, ContainerContract $default = null, string $containerType = '\\WaterUpApi\\Helper\\ParameterContainer'): ?ContainerContract
    {
        if ($this->has($index))
        {
            try
